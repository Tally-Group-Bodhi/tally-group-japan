    ...
    <footer className="bg-navy-dark text-white pt-[72px] pb-6">
      <div className="max-w-[1600px] mx-auto px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr_1fr] gap-[48px] pb-[48px] border-b border-white/10">
            ...
            <FooterCol title="Company" links={companyLinks} href={href} />
            <FooterCol title="Resources" links={resourceLinks} href={href} />
            <FooterCol title="Legal" links={legalLinks} href={href} />
          </div>
  ...
  return (
    <div>
      ...
      <ul className="flex flex-col gap-[10px]">
        {links.map((link) => (
          <li key={link}>
            <Link href={linkPaths[link] ? href(linkPaths[link]) : "#"} className="text-sm text-white/75 hover:text-white transition-colors">
              {link}
            </Link>
          </li>
        ))}
