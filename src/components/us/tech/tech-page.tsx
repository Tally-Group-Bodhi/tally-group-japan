...
type TechPrincipleGridProps = {
  surface?: "soft" | "outline";
} & (
  | { cards: PrincipleCard[]; columns?: never }
  | { columns: [PrincipleCard[], PrincipleCard[]]; cards?: never }
);

export function TechPrincipleGrid(props: TechPrincipleGridProps) {
  const { surface = "soft" } = props;
  const cardClass =
    surface === "outline"
  ...
  // last:grow stretches the final card in each column so columns end at the same height.
  ...
  const [leftCol, rightCol] = props.columns
    ? props.columns
    : [
        props.cards!.filter((_, i) => i % 2 === 0),
        props.cards!.filter((_, i) => i % 2 === 1),
      ];
